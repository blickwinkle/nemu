...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // J type
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + imm; IFDEF(CONFIG_ETRACE, call_ftrace(s->pc, s->dnpc)));
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, csrrw(&s->dnpc, NULL, CSR_MEPC); IFDEF(CONFIG_ETRACE, ret_ftrace(s->pc, s->dnpc)));
  // B type
